...
- fix sync race condition where initial failout of sync dooms workers to idle forever until reboot
...
- make LazyVectorCommitmentTree::commit retry-safe by deferring dirty-state clearing until after transaction commit
- make compute_shard_root read-only to prevent leaking writes outside frame transactions
...
- consensus: rejoin a lagging archive by syncing proposals from peers (implement GlobalService.GetGlobalProposal, persist proposer vote, add catch-up task that pulls missing proposals and submits them into consensus loop)
...
- no commit messages provided for extraction
