...
- adjusted estimation behavior for proper ring position and membership set calculation
...
- improved logging for migration
...
- fix stale worker proposal edge and add full sanity check on join before submission
...
- fix panic on shutdown, libp2p discovery picking inaccessible peers, coverage event check not in shutdown logic, and app shard worker behavior for prover root reconciliation
