...
> **Note (updated May 2026):** This section describes the initial network launch (version 1.x/2.0 era, 2023–2024). Quilibrium 2.1 was released to mainnet on April 14, 2025, which is the current mainnet version. As of May 2026, the network is live on mainnet (QNS, QStorage function) but **tokens, transactions, mining rewards, and full token utility are NOT yet functional** — these require the shard-out transition to complete. The beacon is still active during the shard-out migration. See recent Discord dev-updates for current status.
