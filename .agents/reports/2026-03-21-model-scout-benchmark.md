...
|------|-------|------|-----|-----|-------------|-------|------------|
| 1 | **DeepSeek R1 0528** | 685B MoE | 163k | Yes | 76,131 | **22/27** | PRICIER |
...
**DeepSeek R1 0528** — Reasoning model, strongest overall. Excellent at hallucination detection and correction handling. However, it's a reasoning model (slower, more verbose due to chain-of-thought). May not be ideal as a primary chat model due to latency.

**Qwen3 Coder Next** (480B) — Surprisingly the joint-best performer despite being labeled a "coder" model. 262k context window (vs 163k for DeepSeek), TEE, and cheaper on pay-as-you-go. Strong general Q&A capability.
...
3. **DeepSeek R1 0528** — Highest score tied with Qwen3 Coder, but reasoning model = higher latency. Better suited as a "deep reasoning" fallback than primary.
