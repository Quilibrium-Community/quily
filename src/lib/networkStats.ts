...
export function formatDiscordStats(snapshot: NetworkSnapshot, history: NetworkSnapshot[]): string {
  ...
  msg += `⚠️  Warning (<6)    ${pad(snapshot.warning.toLocaleString('en-US'), 6)}  (${pct(snapshot.warning, total)})\n`;
  ...
  msg += `🟢 Active     ${pad(snapshot.workersActive.toLocaleString('en-US'), 8)}\n`;
  msg += `🔄 Joining    ${pad(snapshot.workersJoining.toLocaleString('en-US'), 8)}\n`;
  msg += `🔻 Leaving    ${pad(snapshot.workersLeaving.toLocaleString('en-US'), 8)}\n`;
